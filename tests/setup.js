...
beforeAll(async () => {
  ...
  let workerScript;
  try {
    workerScript = readFileSync(join(__dirname, '../dist/gateway.js'), 'utf8');
  } catch (error) {
    // Fallback to a simple worker for testing
    workerScript = `
      export default {
  ...
  miniflare = new Miniflare({
    ...
    durableObjects: {
      SHARD: {
        ...
        scriptName: undefined, // Use main script
      },
    },
 ...
 */
export function createSQLRequest(sql, params, hints) {
...
export class PerformanceTestUtils {
  ...
  static assertPerformance(executionTime, maxMs, operationName) {
